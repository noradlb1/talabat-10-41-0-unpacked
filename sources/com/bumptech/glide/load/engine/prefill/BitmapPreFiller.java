...
public final class BitmapPreFiller {
    ...
    public PreFillQueue generateAllocationOrder(PreFillType... preFillTypeArr) {
        ...
        for (PreFillType weight : preFillTypeArr) {
            i11 += weight.getWeight();
        }
        float f11 = ((float) maxSize) / ((float) i11);
        HashMap hashMap = new HashMap();
        for (PreFillType preFillType : preFillTypeArr) {
            hashMap.put(preFillType, Integer.valueOf(Math.round(((float) preFillType.getWeight()) * f11) / getSizeInBytes(preFillType)));
        }
