...
class CrashlyticsController {
    ...
    public static List<NativeSessionFile> getNativeSessionFiles(NativeSessionFileProvider nativeSessionFileProvider, String str, FileStore fileStore2, byte[] bArr) {
        ...
        arrayList.add(new FileBackedNativeSessionFile("session_meta_file", SDKCoreEvent.Session.TYPE_SESSION, nativeSessionFileProvider.getSessionFile()));
    ...
    private void writeApplicationExitInfoEventIfRelevant(String str) {
        ...
        if (i11 >= 30) {
            List a11 = ((ActivityManager) this.context.getSystemService("activity")).getHistoricalProcessExitReasons((String) null, 0, 0);
            if (a11.size() != 0) {
                this.reportingCoordinator.persistRelevantAppExitInfoEvent(str, a11, new LogFileManager(this.fileStore, str), UserMetadata.loadFromExistingSession(str, this.fileStore, this.backgroundWorker));
                return;
            }
            Logger logger = Logger.getLogger();
    ...
    public synchronized void handleUncaughtException(@NonNull SettingsProvider settingsProvider2, @NonNull Thread thread, @NonNull Throwable th2, boolean z11) {
        Logger logger = Logger.getLogger();
