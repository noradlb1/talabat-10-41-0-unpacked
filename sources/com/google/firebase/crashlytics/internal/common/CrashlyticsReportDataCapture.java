...
public class CrashlyticsReportDataCapture {
    ...
    private CrashlyticsReport.Session.Event.Device populateEventDeviceData(int i11) {
        ...
        if (batteryLevel != null) {
            d11 = Double.valueOf(batteryLevel.doubleValue());
        } else {
            d11 = null;
        }
        ...
        return CrashlyticsReport.Session.Event.Device.builder().setBatteryLevel(d11).setBatteryVelocity(batteryVelocity).setProximityOn(proximitySensorEnabled).setOrientation(i11).setRamUsed(totalRamInBytes).setDiskUsed(CommonUtils.calculateUsedDiskSpaceInBytes(Environment.getDataDirectory().getPath())).build();
    }
    ...
    private CrashlyticsReport.Session.Event.Application.Execution.Thread.Frame populateFrameData(StackTraceElement stackTraceElement, CrashlyticsReport.Session.Event.Application.Execution.Thread.Frame.Builder builder) {
        ...
        if (stackTraceElement.isNativeMethod()) {
            j11 = Math.max((long) stackTraceElement.getLineNumber(), 0);
        } else {
            j11 = 0;
        }
        String str = stackTraceElement.getClassName() + RealDiscoveryConfigurationRepository.VERSION_DELIMETER + stackTraceElement.getMethodName();
        String fileName = stackTraceElement.getFileName();
        if (!stackTraceElement.isNativeMethod() && stackTraceElement.getLineNumber() > 0) {
            j12 = (long) stackTraceElement.getLineNumber();
        }
        return builder.setPc(j11).setSymbol(str).setFile(fileName).setOffset(j12).build();
    }
