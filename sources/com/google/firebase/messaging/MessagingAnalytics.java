...
public class MessagingAnalytics {
    ...
    public static long getProjectNumber(Bundle bundle) {
        if (bundle.containsKey(Constants.MessagePayloadKeys.SENDER_ID)) {
            try {
                ...
                Log.w(Constants.TAG, "error parsing project number", e11);
            }
        }
        ...
        if (gcmSenderId != null) {
            try {
                ...
                Log.w(Constants.TAG, "error parsing sender ID", e12);
            }
        }
        ...
        if (!applicationId.startsWith("1:")) {
            ...
            try {
                ...
                Log.w(Constants.TAG, "error parsing app ID", e14);
                return 0;
            }
        }
    }
    ...
    private static void logToFirelog(MessagingClientEvent.Event event, Intent intent, @Nullable TransportFactory transportFactory) {
        ...
        if (eventToProto != null) {
            try {
                transportFactory.getTransport(Constants.FirelogAnalytics.FCM_LOG_SOURCE, MessagingClientEventExtension.class, Encoding.of("proto"), new n()).send(Event.ofData(MessagingClientEventExtension.newBuilder().setMessagingClientEvent(eventToProto).build()));
            } catch (RuntimeException e11) {
                Log.w(Constants.TAG, "Failed to send big query analytics payload.", e11);
            }
        }
    }
        ...
        try {
            ...
            if (messageTime != null) {
                try {
                    ...
                    Log.w(Constants.TAG, "Error while parsing timestamp in GCM event", e11);
                }
            }
            ...
            if (useDeviceTime != null) {
                try {
                    ...
                    Log.w(Constants.TAG, "Error while parsing use_device_time in GCM event", e12);
                }
            }
