...
public class GaugeManager {
    ...
    private long getCpuGaugeCollectionFrequencyMs(ApplicationProcessState applicationProcessState2) {
        long j11;
        int i11 = AnonymousClass1.$SwitchMap$com$google$firebase$perf$v1$ApplicationProcessState[applicationProcessState2.ordinal()];
        if (i11 == 1) {
            j11 = this.configResolver.getSessionsCpuCaptureFrequencyBackgroundMs();
        } else if (i11 != 2) {
            j11 = -1;
        } else {
            j11 = this.configResolver.getSessionsCpuCaptureFrequencyForegroundMs();
        }
        if (CpuGaugeCollector.isInvalidCollectionFrequency(j11)) {
            return -1;
        }
        return j11;
    }
    ...
    private long getMemoryGaugeCollectionFrequencyMs(ApplicationProcessState applicationProcessState2) {
        ...
        int i11 = AnonymousClass1.$SwitchMap$com$google$firebase$perf$v1$ApplicationProcessState[applicationProcessState2.ordinal()];
        if (i11 == 1) {
            j11 = this.configResolver.getSessionsMemoryCaptureFrequencyBackgroundMs();
        } else if (i11 != 2) {
            j11 = -1;
        } else {
            j11 = this.configResolver.getSessionsMemoryCaptureFrequencyForegroundMs();
        }
        if (MemoryGaugeCollector.isInvalidCollectionFrequency(j11)) {
            return -1;
        }
        return j11;
    }
    ...
    private boolean startCollectingCpuMetrics(long j11, Timer timer) {
        ...
        this.cpuGaugeCollector.get().startCollecting(j11, timer);
        return true;
    }
    ...
    private boolean startCollectingMemoryMetrics(long j11, Timer timer) {
        ...
        this.memoryGaugeCollector.get().startCollecting(j11, timer);
        return true;
    }
    ...
    public void startCollectingGauges(PerfSession perfSession, ApplicationProcessState applicationProcessState2) {
        ...
        try {
            ...
            this.gaugeManagerDataCollectionJob = this.gaugeManagerExecutor.get().scheduleAtFixedRate(new d(this, sessionId2, applicationProcessState2), j11, j11, TimeUnit.MILLISECONDS);
        } catch (RejectedExecutionException e11) {
            AndroidLogger androidLogger = logger;
            androidLogger.warn("Unable to start collecting Gauges: " + e11.getMessage());
        }
    }
