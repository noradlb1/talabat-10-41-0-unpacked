...
public class Trace extends AppStateUpdateHandler implements Parcelable, FirebasePerformanceAttributable, SessionAwareObject {
    ...
    private void checkAttribute(@NonNull String str, @NonNull String str2) {
        if (isStopped()) {
            throw new IllegalArgumentException(String.format(Locale.ENGLISH, "Trace '%s' has been stopped", new Object[]{this.f47452name}));
    ...
    public void finalize() throws Throwable {
        try {
            if (isActive()) {
                logger.warn("Trace '%s' is started but not stopped when it is destructed!", this.f47452name);
    ...
    public void incrementMetric(@NonNull String str, long j11) {
        ...
        if (validateMetricName != null) {
            ...
            logger.warn("Cannot increment metric '%s' for trace '%s' because it's not started", str, this.f47452name);
        } else if (isStopped()) {
            logger.warn("Cannot increment metric '%s' for trace '%s' because it's been stopped", str, this.f47452name);
        } else {
            Counter obtainOrCreateCounterByName = obtainOrCreateCounterByName(str.trim());
            obtainOrCreateCounterByName.increment(j11);
            logger.debug("Incrementing metric '%s' to %d on trace '%s'", str, Long.valueOf(obtainOrCreateCounterByName.getCount()), this.f47452name);
        }
    }
    ...
    public void putAttribute(@NonNull String str, @NonNull String str2) {
        ...
        try {
            ...
            logger.debug("Setting attribute '%s' to '%s' on trace '%s'", str, str2, this.f47452name);
            z11 = true;
        } catch (Exception e11) {
            logger.error("Can not set attribute '%s' with value '%s' (%s)", str, str2, e11.getMessage());
        }
        if (z11) {
    ...
    public void putMetric(@NonNull String str, long j11) {
        ...
        if (validateMetricName != null) {
            ...
            logger.warn("Cannot set value for metric '%s' for trace '%s' because it's not started", str, this.f47452name);
        } else if (isStopped()) {
            logger.warn("Cannot set value for metric '%s' for trace '%s' because it's been stopped", str, this.f47452name);
        } else {
            obtainOrCreateCounterByName(str.trim()).setCount(j11);
            logger.debug("Setting metric '%s' to '%s' on trace '%s'", str, Long.valueOf(j11), this.f47452name);
        }
    }
    ...
    public void start() {
        ...
        String validateTraceName = PerfMetricValidator.validateTraceName(this.f47452name);
        if (validateTraceName != null) {
            logger.error("Cannot start trace '%s'. Trace name is invalid.(%s)", this.f47452name, validateTraceName);
        } else if (this.startTime != null) {
            logger.error("Trace '%s' has already started, should not start again!", this.f47452name);
        } else {
    ...
    public void stop() {
        if (!hasStarted()) {
            logger.error("Trace '%s' has not been started so unable to stop!", this.f47452name);
        } else if (isStopped()) {
            logger.error("Trace '%s' has already stopped, should not stop again!", this.f47452name);
        } else {
    ...
    public void writeToParcel(@NonNull Parcel parcel, int i11) {
        ...
        parcel.writeString(this.f47452name);
    ...
    @NonNull
    @VisibleForTesting
    public static synchronized Trace getTrace(@NonNull String str, @NonNull TransportManager transportManager2, @NonNull Clock clock2, @NonNull AppStateMonitor appStateMonitor) {
        Trace trace;
        synchronized (Trace.class) {
            Map<String, Trace> map = traceNameToTraceMap;
            trace = map.get(str);
