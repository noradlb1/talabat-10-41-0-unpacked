...
public class WebSocket {
        ...
        if (this.state != State.DISCONNECTED) {
            ...
            if (this.socket != null) {
                try {
                    ...
                    this.eventHandler.onError(new WebSocketException("Failed to close", e11));
                }
            }
    ...
    private Socket createSocket() {
        ...
        if (scheme != null && scheme.equals("ws")) {
            ...
            try {
                ...
                throw new WebSocketException("error while creating socket to " + this.url, e12);
            }
            ...
            try {
                ...
                this.logger.debug("Failed to initialize SSL session cache", e13, new Object[0]);
            }
            try {
                ...
                throw new WebSocketException("error while creating secure socket to " + this.url, e15);
            }
        }
    }
    ...
    private void sendCloseHandshake() {
        try {
            ...
            this.eventHandler.onError(new WebSocketException("Failed to send close frame", e11));
        }
    }
    ...
    public synchronized void close() {
        int i11 = AnonymousClass3.$SwitchMap$com$google$firebase$database$tubesock$WebSocket$State[this.state.ordinal()];
        if (i11 == 1) {
            this.state = State.DISCONNECTED;
        } else if (i11 == 2) {
            closeSocket();
    ...
    private synchronized void send(byte b11, byte[] bArr) {
        if (this.state != State.CONNECTED) {
            this.eventHandler.onError(new WebSocketException("error while sending data: not connected"));
        } else {
            try {
                this.writer.send(b11, true, bArr);
            } catch (IOException e11) {
                this.eventHandler.onError(new WebSocketException("Failed to send frame", e11));
                close();
            }
        }
        return;
