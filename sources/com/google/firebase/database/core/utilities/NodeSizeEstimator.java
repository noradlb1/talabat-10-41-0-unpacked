...
public class NodeSizeEstimator {
    ...
    private static long estimateLeafNodeSize(LeafNode<?> leafNode) {
        ...
        if (!(leafNode instanceof DoubleNode) && !(leafNode instanceof LongNode)) {
            if (leafNode instanceof BooleanNode) {
                ...
                j11 = ((long) ((String) leafNode.getValue()).length()) + 2;
            } else {
        ...
        return j11 + 24 + estimateLeafNodeSize((LeafNode) leafNode.getPriority());
    }
    ...
    public static long estimateSerializedNodeSize(Node node) {
        ...
        while (it.hasNext()) {
            ...
            j11 = j11 + ((long) namedNode.getName().asString().length()) + 4 + estimateSerializedNodeSize(namedNode.getNode());
        }
        if (!node.getPriority().isEmpty()) {
            return j11 + 12 + estimateLeafNodeSize((LeafNode) node.getPriority());
        }
        return j11;
    }
    ...
    public static int nodeCount(Node node) {
        ...
        Iterator it = node.iterator();
        while (it.hasNext()) {
            i11 += nodeCount(((NamedNode) it.next()).getNode());
        }
        return i11;
    }
}
