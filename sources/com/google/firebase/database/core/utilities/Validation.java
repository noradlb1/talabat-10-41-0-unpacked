...
public class Validation {
    ...
    private static boolean isWritableKey(String str) {
        if (str == null || str.length() <= 0 || (!str.equals(".value") && !str.equals(".priority") && (str.startsWith(RealDiscoveryConfigurationRepository.VERSION_DELIMETER) || INVALID_KEY_REGEX.matcher(str).find()))) {
            return false;
        }
        return true;
    }
    ...
    private static boolean isWritablePath(Path path) {
        ...
        if (front == null || !front.asString().startsWith(RealDiscoveryConfigurationRepository.VERSION_DELIMETER)) {
            return true;
        }
        return false;
    }
    ...
    private static void validateDoubleValue(double d11) {
