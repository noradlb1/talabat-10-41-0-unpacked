...
public final class CompoundWrite implements Iterable<Map.Entry<Path, Node>> {
    ...
    private Node applySubtreeWrite(Path path, ImmutableTree<Node> immutableTree, Node node) {
        ...
        while (it.hasNext()) {
            ...
            if (childKey.isPriorityChildName()) {
                ...
                Utilities.hardAssert(z11, "Priority writes must always be leaf nodes");
    ...
    public Map<String, Object> getValue(final boolean z11) {
        ...
        this.writeTree.foreach(new ImmutableTree.TreeVisitor<Node, Void>() {
            public Void onNodeValue(Path path, Node node, Void voidR) {
                hashMap.put(path.wireFormat(), node.getValue(z11));
                return null;
            }
        });
        return hashMap;
