...
public class SyncTree {
    ...
    public List<? extends Event> applyTaggedOperation(QuerySpec querySpec, Operation operation) {
        ...
        Utilities.hardAssert(z11, "Missing sync point for query tag that we're tracking");
    ...
    public Tag getNextQueryTag() {
        long j11 = this.nextQueryTag;
        this.nextQueryTag = 1 + j11;
        return new Tag(j11);
    }
    ...
    public /* synthetic */ Node lambda$getServerValue$0(QuerySpec querySpec) throws Exception {
        ...
        return syncPoint.getView(querySpec, this.pendingWriteTree.childWrites(path), new CacheNode(from, z11, false)).getCompleteNode();
    }
    ...
    public List<? extends Event> ackUserWrite(long j11, boolean z11, boolean z12, Clock clock) {
        ...
        return (List) this.persistenceManager.runInTransaction(new Callable<List<? extends Event>>() {
            public List<? extends Event> call() {
                if (z13) {
                    SyncTree.this.persistenceManager.removeUserWrite(j12);
                }
                UserWriteRecord write = SyncTree.this.pendingWriteTree.getWrite(j12);
                boolean removeWrite = SyncTree.this.pendingWriteTree.removeWrite(j12);
                if (write.isVisible() && !z14) {
                ...
                return SyncTree.this.applyOperationToSyncPoints(new AckUserWrite(write.getPath(), emptyInstance, z14));
            }
        });
    }
    ...
    public List<? extends Event> applyTaggedRangeMerges(Path path, List<RangeMerge> list, Tag tag) {
        ...
        Utilities.hardAssert(z12, "Missing view for query tag that we're tracking");
    ...
    public List<? extends Event> applyUserMerge(Path path, CompoundWrite compoundWrite, CompoundWrite compoundWrite2, long j11, boolean z11) {
        ...
        return (List) this.persistenceManager.runInTransaction(new Callable<List<? extends Event>>() {
            public List<? extends Event> call() throws Exception {
                if (z12) {
                    SyncTree.this.persistenceManager.saveUserMerge(path2, compoundWrite3, j12);
                }
                SyncTree.this.pendingWriteTree.addMerge(path2, compoundWrite4, Long.valueOf(j12));
                return SyncTree.this.applyOperationToSyncPoints(new Merge(OperationSource.USER, path2, compoundWrite4));
            }
        });
    }

    public List<? extends Event> applyUserOverwrite(Path path, Node node, Node node2, long j11, boolean z11, boolean z12) {
        boolean z13;
        if (z11 || !z12) {
            z13 = true;
        } else {
        ...
        Utilities.hardAssert(z13, "We shouldn't be persisting non-visible writes.");
        ...
        return (List) this.persistenceManager.runInTransaction(new Callable<List<? extends Event>>() {
            public List<? extends Event> call() {
                if (z14) {
                    SyncTree.this.persistenceManager.saveUserOverwrite(path2, node3, j12);
                }
                SyncTree.this.pendingWriteTree.addOverwrite(path2, node4, Long.valueOf(j12), z15);
                if (!z15) {
                    return Collections.emptyList();
                }
    ...
    public Node getServerValue(QuerySpec querySpec) {
        return (Node) this.persistenceManager.runInTransaction(new b(this, querySpec));
    }
    ...
    public List<? extends Event> addEventRegistration(@NotNull final EventRegistration eventRegistration, final boolean z11) {
        return (List) this.persistenceManager.runInTransaction(new Callable<List<? extends Event>>() {
            public List<? extends Event> call() {
                CacheNode cacheNode;
    ...
    public void keepSynced(QuerySpec querySpec, boolean z11, boolean z12) {
        if (z11 && !this.keepSyncedQueries.contains(querySpec)) {
            addEventRegistration(new KeepSyncedEventRegistration(querySpec), z12);
            this.keepSyncedQueries.add(querySpec);
        } else if (!z11 && this.keepSyncedQueries.contains(querySpec)) {
            removeEventRegistration(new KeepSyncedEventRegistration(querySpec), z12);
    ...
    public List<Event> removeEventRegistration(@NotNull EventRegistration eventRegistration, boolean z11) {
        return removeEventRegistration(eventRegistration.getQuerySpec(), eventRegistration, (DatabaseError) null, z11);
    }

    private List<Event> removeEventRegistration(@NotNull QuerySpec querySpec, @Nullable EventRegistration eventRegistration, @Nullable DatabaseError databaseError, boolean z11) {
        ...
        return (List) this.persistenceManager.runInTransaction(new Callable<List<Event>>() {
            public List<Event> call() {
                ...
                if (syncPoint != null && (querySpec2.isDefault() || syncPoint.viewExistsForQuery(querySpec2))) {
                    ...
                    ImmutableTree access$600 = SyncTree.this.syncPointTree;
                    boolean z12 = access$600.getValue() != null && ((SyncPoint) access$600.getValue()).hasCompleteView();
                    Iterator<ChildKey> it2 = path.iterator();
                    while (it2.hasNext()) {
                        access$600 = access$600.getChild(it2.next());
                        z12 = z12 || (access$600.getValue() != null && ((SyncPoint) access$600.getValue()).hasCompleteView());
                        if (!z12) {
                    ...
                    if (!z12 && !first.isEmpty() && databaseError2 == null) {
                        if (z11) {
