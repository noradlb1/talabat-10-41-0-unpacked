...
public class View {
    ...
    public OperationResult applyOperation(Operation operation, WriteTreeRef writeTreeRef, Node node) {
        ...
        if (operation.getType() == Operation.OperationType.Merge && operation.getSource().getQueryParams() != null) {
            ...
            Utilities.hardAssert(z11, "We should always have a full cache before handling merges");
            if (this.viewCache.getCompleteEventSnap() != null) {
                z12 = true;
            } else {
                z12 = false;
            }
            Utilities.hardAssert(z12, "Missing event cache, even though we have a server cache");
        }
        ...
        Utilities.hardAssert(z13, "Once a server snap is complete, it should never go back");
    ...
    public List<Event> removeEventRegistration(@Nullable EventRegistration eventRegistration, DatabaseError databaseError) {
        ...
        if (databaseError != null) {
            ...
            Utilities.hardAssert(z11, "A cancel should cancel all event registrations");
        ...
        if (eventRegistration != null) {
            int i12 = -1;
            while (true) {
                if (i11 >= this.eventRegistrations.size()) {
                    i11 = i12;
                    break;
                }
                EventRegistration eventRegistration2 = this.eventRegistrations.get(i11);
            ...
            if (i11 != -1) {
                this.eventRegistrations.remove(i11);
                this.eventRegistrations.get(i11).zombify();
            }
        } else {
