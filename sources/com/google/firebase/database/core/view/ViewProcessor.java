...
public class ViewProcessor {
    private static NodeFilter.CompleteChildSource NO_COMPLETE_SOURCE = new NodeFilter.CompleteChildSource() {
        public NamedNode getChildAfterChild(Index index, NamedNode namedNode, boolean z11) {
            return null;
        }
    ...
    public static class WriteTreeCompleteChildSource implements NodeFilter.CompleteChildSource {
        ...
        public NamedNode getChildAfterChild(Index index, NamedNode namedNode, boolean z11) {
            ...
            return this.writes.calcNextNodeAfterPost(node, namedNode, z11, index);
        }
    ...
    private ViewCache applyServerMerge(ViewCache viewCache, Path path, CompoundWrite compoundWrite, WriteTreeRef writeTreeRef, Node node, boolean z11, ChildChangeAccumulator childChangeAccumulator) {
        boolean z12;
        ...
        Utilities.hardAssert(z12, "Can't have a merge that is an overwrite");
        if (path.isEmpty()) {
        ...
        for (Map.Entry next : childCompoundWrites.entrySet()) {
            ...
            if (node2.hasChild(childKey)) {
                viewCache2 = applyServerOverwrite(viewCache2, new Path(childKey), ((CompoundWrite) next.getValue()).apply(node2.getImmediateChild(childKey)), writeTreeRef, node, z11, childChangeAccumulator);
            }
        }
        ...
        for (Map.Entry next2 : childCompoundWrites.entrySet()) {
            ...
            if (!node2.hasChild(childKey2) && !z13) {
                viewCache3 = applyServerOverwrite(viewCache3, new Path(childKey2), ((CompoundWrite) next2.getValue()).apply(node2.getImmediateChild(childKey2)), writeTreeRef, node, z11, childChangeAccumulator);
            }
        }
        return viewCache3;
    }

    private ViewCache applyServerOverwrite(ViewCache viewCache, Path path, Node node, WriteTreeRef writeTreeRef, Node node2, boolean z11, ChildChangeAccumulator childChangeAccumulator) {
        ...
        if (path.isEmpty()) {
            ...
            ViewCache updateServerSnap = viewCache2.updateServerSnap(indexedNode, z12, nodeFilter.filtersNodes());
        ...
        Path path2 = path;
        z12 = false;
        ViewCache updateServerSnap2 = viewCache2.updateServerSnap(indexedNode, z12, nodeFilter.filtersNodes());
    ...
    private ViewCache generateEventCacheAfterServerEvent(ViewCache viewCache, Path path, WriteTreeRef writeTreeRef, NodeFilter.CompleteChildSource completeChildSource, ChildChangeAccumulator childChangeAccumulator) {
        ...
        if (path.isEmpty()) {
            ...
            if (front.isPriorityChildName()) {
                ...
                Utilities.hardAssert(z11, "Can't have a priority with additional path components");
        ...
        return viewCache2.updateEventSnap(indexedNode, z12, this.filter.filtersNodes());
    }
    ...
    private ViewCache listenComplete(ViewCache viewCache, Path path, WriteTreeRef writeTreeRef, Node node, ChildChangeAccumulator childChangeAccumulator) {
        ...
        return generateEventCacheAfterServerEvent(viewCache.updateServerSnap(indexedNode, z11, serverCache.isFiltered()), path, writeTreeRef, NO_COMPLETE_SOURCE, childChangeAccumulator);
    }
    ...
    private void maybeAddValueEvent(ViewCache viewCache, ViewCache viewCache2, List<Change> list) {
        ...
        if (eventCache.isFullyInitialized()) {
            ...
            if (!list.isEmpty() || !viewCache.getEventCache().isFullyInitialized() || ((z11 && !eventCache.getNode().equals(viewCache.getCompleteEventSnap())) || !eventCache.getNode().getPriority().equals(viewCache.getCompleteEventSnap().getPriority()))) {
    ...
    public ProcessorResult applyOperation(ViewCache viewCache, Operation operation, WriteTreeRef writeTreeRef, Node node) {
        ...
        int i11 = AnonymousClass2.$SwitchMap$com$google$firebase$database$core$operation$Operation$OperationType[operation.getType().ordinal()];
        if (i11 == 1) {
            ...
            if (overwrite.getSource().isFromUser()) {
                ...
                viewCache2 = applyServerOverwrite(viewCache, overwrite.getPath(), overwrite.getSnapshot(), writeTreeRef, node, z11, childChangeAccumulator);
            }
        } else if (i11 == 2) {
            ...
            if (merge.getSource().isFromUser()) {
                ...
                viewCache2 = applyServerMerge(viewCache, merge.getPath(), merge.getChildren(), writeTreeRef, node, z12, childChangeAccumulator);
            }
        } else if (i11 == 3) {
    ...
    public ViewCache revertUserWrite(ViewCache viewCache, Path path, WriteTreeRef writeTreeRef, Node node, ChildChangeAccumulator childChangeAccumulator) {
        ...
        return viewCache.updateEventSnap(indexedNode, z11, this.filter.filtersNodes());
    }
}
