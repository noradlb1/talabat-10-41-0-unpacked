...
public class TrackedQueryManager {
    ...
    public TrackedQueryManager(PersistenceStorageEngine persistenceStorageEngine, LogWrapper logWrapper, Clock clock2) {
        ...
        for (TrackedQuery next : persistenceStorageEngine.loadTrackedQueries()) {
            this.currentQueryId = Math.max(next.f47411id + 1, this.currentQueryId);
    ...
    private static void assertValidTrackedQuery(QuerySpec querySpec) {
        ...
        Utilities.hardAssert(z11, "Can't have tracked non-default query that loads all data");
    }
    ...
    private void cacheTrackedQuery(TrackedQuery trackedQuery) {
        ...
        if (trackedQuery2 == null || trackedQuery2.f47411id == trackedQuery.f47411id) {
            z11 = true;
        } else {
            z11 = false;
        }
    ...
    private static long calculateCountToPrune(CachePolicy cachePolicy, long j11) {
        return j11 - Math.min((long) Math.floor((double) (((float) j11) * (1.0f - cachePolicy.getPercentOfQueriesToPruneAtOnce()))), cachePolicy.getMaxNumberOfQueriesToKeep());
    }
        ...
        if (map != null) {
            for (TrackedQuery trackedQuery : map.values()) {
                if (!trackedQuery.querySpec.loadsAllData()) {
                    hashSet.add(Long.valueOf(trackedQuery.f47411id));
                }
            }
        }
        return hashSet;
    ...
    private void setQueryActiveFlag(QuerySpec querySpec, boolean z11) {
        ...
        if (findTrackedQuery != null) {
            trackedQuery = findTrackedQuery.updateLastUse(millis).setActiveState(z11);
        } else {
            Utilities.hardAssert(z11, "If we're setting the query to inactive, we should already be tracking it!");
            long j11 = this.currentQueryId;
            this.currentQueryId = 1 + j11;
            trackedQuery = new TrackedQuery(j11, normalizeQuery, millis, false, z11);
        }
    ...
    public void ensureCompleteTrackedQuery(Path path) {
        ...
        if (!includedInDefaultCompleteQuery(path)) {
            ...
            if (findTrackedQuery == null) {
                long j11 = this.currentQueryId;
                this.currentQueryId = 1 + j11;
                trackedQuery = new TrackedQuery(j11, defaultQueryAtPath, this.clock.millis(), true, false);
            } else {
    ...
    public PruneForest pruneOldQueries(CachePolicy cachePolicy) {
        ...
        for (int i11 = 0; ((long) i11) < calculateCountToPrune; i11++) {
            TrackedQuery trackedQuery = queriesMatching.get(i11);
            pruneForest = pruneForest.prune(trackedQuery.querySpec.getPath());
            removeTrackedQuery(trackedQuery.querySpec);
        }
        for (int i12 = (int) calculateCountToPrune; i12 < queriesMatching.size(); i12++) {
            pruneForest = pruneForest.keep(queriesMatching.get(i12).querySpec.getPath());
        }
    ...
    public void removeTrackedQuery(QuerySpec querySpec) {
        ...
        Utilities.hardAssert(z11, "Query must exist to be removed.");
        this.storageLayer.deleteTrackedQuery(findTrackedQuery.f47411id);
        Map map = this.trackedQueryTree.get(normalizeQuery.getPath());
    ...
    public void verifyCache() {
        ...
        Collections.sort(arrayList, new Comparator<TrackedQuery>() {
            public int compare(TrackedQuery trackedQuery, TrackedQuery trackedQuery2) {
                return Utilities.compareLongs(trackedQuery.f47411id, trackedQuery2.f47411id);
            }
        });
