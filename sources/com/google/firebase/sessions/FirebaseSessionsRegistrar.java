...
public final class FirebaseSessionsRegistrar implements ComponentRegistrar {
    ...
    public static final FirebaseSessions m9376getComponents$lambda0(ComponentContainer componentContainer) {
    ...
    public List<Component<? extends Object>> getComponents() {
        return CollectionsKt__CollectionsKt.listOf(Component.builder(FirebaseSessions.class).name(LIBRARY_NAME).add(Dependency.required((Qualified<?>) firebaseApp)).add(Dependency.required((Qualified<?>) firebaseInstallationsApi)).add(Dependency.required((Qualified<?>) backgroundDispatcher)).add(Dependency.required((Qualified<?>) blockingDispatcher)).add(Dependency.requiredProvider((Qualified<?>) transportFactory)).factory(new b()).build(), LibraryVersionComponent.create(LIBRARY_NAME, BuildConfig.VERSION_NAME));
    }
}
