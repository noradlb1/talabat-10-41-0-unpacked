...
public class ShadowDrawableWrapper extends DrawableWrapper {
    ...
    private void drawShadow(@NonNull Canvas canvas) {
        ...
        float f17 = f15 * 2.0f;
        if (this.contentBounds.width() - f17 > 0.0f) {
            z11 = true;
        } else {
            z11 = false;
        }
        if (this.contentBounds.height() - f17 > 0.0f) {
            z12 = true;
        } else {
            z12 = false;
        }
        ...
        float f21 = f15 / ((f18 - (SHADOW_TOP_SCALE * f18)) + f15);
    ...
    public float getMinHeight() {
        float f11 = this.rawMaxShadowSize;
        return (Math.max(f11, this.cornerRadius + ((f11 * SHADOW_MULTIPLIER) / 2.0f)) * 2.0f) + (this.rawMaxShadowSize * SHADOW_MULTIPLIER * 2.0f);
    }
