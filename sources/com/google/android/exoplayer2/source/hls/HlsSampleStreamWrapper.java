...
final class HlsSampleStreamWrapper implements Loader.Callback<Chunk>, Loader.ReleaseCallback, SequenceableLoader, ExtractorOutput, SampleQueue.UpstreamFormatChangedListener {
    ...
    public HlsSampleStreamWrapper(String str, int i11, Callback callback2, HlsChunkSource hlsChunkSource, Map<String, DrmInitData> map, Allocator allocator2, long j11, @Nullable Format format, DrmSessionManager drmSessionManager2, DrmSessionEventListener.EventDispatcher eventDispatcher, LoadErrorHandlingPolicy loadErrorHandlingPolicy2, MediaSourceEventListener.EventDispatcher eventDispatcher2, int i12) {
        this.uid = str;
        this.trackType = i11;
        ...
        this.maybeFinishPrepareRunnable = new a(this);
        this.onTracksEndedRunnable = new b(this);
    ...
    private void buildTracksFromSampleStreams() {
        ...
        while (true) {
            ...
            String str = ((Format) Assertions.checkStateNotNull(this.sampleQueues[i14].getUpstreamFormat())).sampleMimeType;
        ...
        for (int i18 = 0; i18 < length; i18++) {
            Format format4 = (Format) Assertions.checkStateNotNull(this.sampleQueues[i18].getUpstreamFormat());
            if (i18 == i13) {
                Format[] formatArr = new Format[i16];
                for (int i19 = 0; i19 < i16; i19++) {
                    Format format5 = trackGroup.getFormat(i19);
                    if (i12 == 1 && (format3 = this.muxedAudioFormat) != null) {
                        format5 = format5.withManifestFormatInfo(format3);
                    }
                    if (i16 == 1) {
                        format2 = format4.withManifestFormatInfo(format5);
                    } else {
                        format2 = deriveFormat(format5, format4, true);
                    }
                    formatArr[i19] = format2;
                }
                trackGroupArr[i18] = new TrackGroup(this.uid, formatArr);
                this.primaryTrackGroupIndex = i18;
            } else {
                if (i12 != 2 || !MimeTypes.isAudio(format4.sampleMimeType)) {
                    format = null;
                } else {
                ...
                sb2.append(i11);
                trackGroupArr[i18] = new TrackGroup(sb2.toString(), deriveFormat(format, format4, false));
            }
        }
    ...
    private static DummyTrackOutput createFakeTrackOutput(int i11, int i12) {
        Log.w(TAG, "Unmapped track with id " + i11 + " of type " + i12);
    ...
    private SampleQueue createSampleQueue(int i11, int i12) {
        ...
        copyOf2[length] = z11;
        this.haveAudioVideoSampleQueues |= z11;
    ...
    private TrackGroupArray createTrackGroupArrayWithDrmInfo(TrackGroup[] trackGroupArr) {
        for (int i11 = 0; i11 < trackGroupArr.length; i11++) {
            ...
            for (int i12 = 0; i12 < trackGroup.length; i12++) {
                ...
                formatArr[i12] = format.copyWithCryptoType(this.drmSessionManager.getCryptoType(format));
            }
            trackGroupArr[i11] = new TrackGroup(trackGroup.f35000id, formatArr);
        }
        return new TrackGroupArray(trackGroupArr);
    }

    private static Format deriveFormat(@Nullable Format format, Format format2, boolean z11) {
        String str;
        String str2;
        int i11;
        int i12;
        ...
        Format.Builder roleFlags = format2.buildUpon().setId(format.f34900id).setLabel(format.label).setLanguage(format.language).setSelectionFlags(format.selectionFlags).setRoleFlags(format.roleFlags);
        if (z11) {
            i11 = format.averageBitrate;
        } else {
            i11 = -1;
        }
        Format.Builder averageBitrate = roleFlags.setAverageBitrate(i11);
        if (z11) {
            i12 = format.peakBitrate;
        } else {
            i12 = -1;
        }
        Format.Builder codecs = averageBitrate.setPeakBitrate(i12).setCodecs(str2);
        if (trackType2 == 2) {
        ...
        if (i13 != -1 && trackType2 == 1) {
            codecs.setChannelCount(i13);
        }
    ...
    public boolean continueLoading(long j11) {
        ...
        hlsChunkSource.getNextChunk(j11, j13, list2, z11, this.nextChunkHolder);
        HlsChunkSource.HlsChunkHolder hlsChunkHolder = this.nextChunkHolder;
        boolean z12 = hlsChunkHolder.endOfStream;
    ...
    public boolean onPlaylistError(Uri uri, LoadErrorHandlingPolicy.LoadErrorInfo loadErrorInfo, boolean z11) {
        ...
        if (z11 || (fallbackSelectionFor = this.loadErrorHandlingPolicy.getFallbackSelectionFor(TrackSelectionUtil.createFallbackOptions(this.chunkSource.getTrackSelection()), loadErrorInfo)) == null || fallbackSelectionFor.type != 2) {
            j11 = -9223372036854775807L;
        } else {
            j11 = fallbackSelectionFor.exclusionDurationMs;
        }
        if (!this.chunkSource.onPlaylistError(uri, j11) || j11 == C.TIME_UNSET) {
            return false;
        }
        return true;
    }
    ...
    public void prepareWithMultivariantPlaylistInfo(TrackGroup[] trackGroupArr, int i11, int... iArr) {
        this.trackGroups = createTrackGroupArrayWithDrmInfo(trackGroupArr);
    ...
    public Loader.LoadErrorAction onLoadError(Chunk chunk, long j11, long j12, IOException iOException, int i11) {
        ...
        LoadErrorHandlingPolicy.LoadErrorInfo loadErrorInfo = new LoadErrorHandlingPolicy.LoadErrorInfo(loadEventInfo, new MediaLoadData(chunk2.type, this.trackType, chunk2.trackFormat, chunk2.trackSelectionReason, chunk2.trackSelectionData, Util.usToMs(chunk2.startTimeUs), Util.usToMs(chunk2.endTimeUs)), iOException2, i11);
