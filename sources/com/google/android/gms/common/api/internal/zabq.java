...
public final class zabq implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, zau {
    ...
    @WorkerThread
    private final void zaE(@Nullable Status status, @Nullable Exception exc, boolean z11) {
        boolean z12;
        Preconditions.checkHandlerThread(this.zaa.zar);
        boolean z13 = false;
        if (status != null) {
    ...
    public final void zaH(int i11) {
        ...
        googleApiManager.zar.sendMessageDelayed(Message.obtain(googleApiManager.zar, 9, this.zad), 5000);
        GoogleApiManager googleApiManager2 = this.zaa;
        googleApiManager2.zar.sendMessageDelayed(Message.obtain(googleApiManager2.zar, 11, this.zad), PayloadController.PAYLOAD_REQUEUE_PERIOD_MS);
        this.zaa.zak.zac();
    ...
    private final boolean zaL(zai zai2) {
        ...
        String name3 = zaB.getName();
        long version = zaB.getVersion();
        Log.w("GoogleApiManager", name2 + " could not execute call because it requires feature (" + name3 + IndicativeSentencesGeneration.DEFAULT_SEPARATOR + version + ").");
        ...
        googleApiManager3.zar.sendMessageDelayed(Message.obtain(googleApiManager3.zar, 16, zabs), PayloadController.PAYLOAD_REQUEUE_PERIOD_MS);
        ...
        if (!this.zac.isConnected() && !this.zac.isConnecting()) {
            try {
                ...
                if (zab2 != 0) {
                    ...
                    Log.w("GoogleApiManager", "The service for " + name2 + " is not available: " + obj);
                    zar(connectionResult, (Exception) null);
                    return;
                }
                GoogleApiManager googleApiManager2 = this.zaa;
    ...
    public final void zas(@NonNull ConnectionResult connectionResult) {
        Preconditions.checkHandlerThread(this.zaa.zar);
        Api.Client client = this.zac;
        String name2 = client.getClass().getName();
        String valueOf = String.valueOf(connectionResult);
        client.disconnect("onSignInFailed for " + name2 + " with " + valueOf);
        zar(connectionResult, (Exception) null);
    }
