package com.google.android.gms.internal.gtm;

import java.util.Arrays;
import java.util.Collection;
import java.util.RandomAccess;

final class zzqm extends zzpo<Double> implements zzsv, RandomAccess {
    private static final zzqm zzaxe;
    private int size;
    private double[] zzaxf;

    static {
        zzqm zzqm = new zzqm(new double[0], 0);
        zzaxe = zzqm;
        zzqm.zzmi();
    }

    public zzqm() {
        this(new double[10], 0);
    }

    private final void zzah(int i11) {
        if (i11 < 0 || i11 >= this.size) {
            throw new IndexOutOfBoundsException(zzai(i11));
        }
    }

    private final String zzai(int i11) {
        int i12 = this.size;
        StringBuilder sb2 = new StringBuilder(35);
        sb2.append("Index:");
        sb2.append(i11);
        sb2.append(", Size:");
        sb2.append(i12);
        return sb2.toString();
    }

    private final void zzc(int i11, double d11) {
        int i12;
        zzmz();
        if (i11 < 0 || i11 > (i12 = this.size)) {
            throw new IndexOutOfBoundsException(zzai(i11));
        }
        double[] dArr = this.zzaxf;
        if (i12 < dArr.length) {
            System.arraycopy(dArr, i11, dArr, i11 + 1, i12 - i11);
        } else {
            double[] dArr2 = new double[(((i12 * 3) / 2) + 1)];
            System.arraycopy(dArr, 0, dArr2, 0, i11);
            System.arraycopy(this.zzaxf, i11, dArr2, i11 + 1, this.size - i11);
            this.zzaxf = dArr2;
        }
        this.zzaxf[i11] = d11;
        this.size++;
        this.modCount++;
    }

    public final /* synthetic */ void add(int i11, Object obj) {
        zzc(i11, ((Double) obj).doubleValue());
    }

    public final boolean addAll(Collection<? extends Double> collection) {
        zzmz();
        zzre.checkNotNull(collection);
        if (!(collection instanceof zzqm)) {
            return super.addAll(collection);
        }
        zzqm zzqm = (zzqm) collection;
        int i11 = zzqm.size;
        if (i11 == 0) {
            return false;
        }
        int i12 = this.size;
        if (Integer.MAX_VALUE - i12 >= i11) {
            int i13 = i12 + i11;
            double[] dArr = this.zzaxf;
            if (i13 > dArr.length) {
                this.zzaxf = Arrays.copyOf(dArr, i13);
            }
            System.arraycopy(zzqm.zzaxf, 0, this.zzaxf, this.size, zzqm.size);
            this.size = i13;
            this.modCount++;
            return true;
        }
        throw new OutOfMemoryError();
    }

    public final boolean equals(Object obj) {
        if (this == obj) {
            return true;
        }
        if (!(obj instanceof zzqm)) {
            return super.equals(obj);
        }
        zzqm zzqm = (zzqm) obj;
        if (this.size != zzqm.size) {
            return false;
        }
        double[] dArr = zzqm.zzaxf;
        for (int i11 = 0; i11 < this.size; i11++) {
            if (Double.doubleToLongBits(this.zzaxf[i11]) != Double.doubleToLongBits(dArr[i11])) {
                return false;
            }
        }
        return true;
    }

    public final /* synthetic */ Object get(int i11) {
        zzah(i11);
        return Double.valueOf(this.zzaxf[i11]);
    }

    public final int hashCode() {
        int i11 = 1;
        for (int i12 = 0; i12 < this.size; i12++) {
            i11 = (i11 * 31) + zzre.zzz(Double.doubleToLongBits(this.zzaxf[i12]));
        }
        return i11;
    }

    public final boolean remove(Object obj) {
        zzmz();
        for (int i11 = 0; i11 < this.size; i11++) {
            if (obj.equals(Double.valueOf(this.zzaxf[i11]))) {
                double[] dArr = this.zzaxf;
                System.arraycopy(dArr, i11 + 1, dArr, i11, (this.size - i11) - 1);
                this.size--;
                this.modCount++;
                return true;
            }
        }
        return false;
    }

    public final void removeRange(int i11, int i12) {
        zzmz();
        if (i12 >= i11) {
            double[] dArr = this.zzaxf;
            System.arraycopy(dArr, i12, dArr, i11, this.size - i12);
            this.size -= i12 - i11;
            this.modCount++;
            return;
        }
        throw new IndexOutOfBoundsException("toIndex < fromIndex");
    }

    public final /* synthetic */ Object set(int i11, Object obj) {
        double doubleValue = ((Double) obj).doubleValue();
        zzmz();
        zzah(i11);
        double[] dArr = this.zzaxf;
        double d11 = dArr[i11];
        dArr[i11] = doubleValue;
        return Double.valueOf(d11);
    }

    public final int size() {
        return this.size;
    }

    public final /* synthetic */ zzrj zzaj(int i11) {
        if (i11 >= this.size) {
            return new zzqm(Arrays.copyOf(this.zzaxf, i11), this.size);
        }
        throw new IllegalArgumentException();
    }

    public final void zzd(double d11) {
        zzc(this.size, d11);
    }

    private zzqm(double[] dArr, int i11) {
        this.zzaxf = dArr;
        this.size = i11;
    }

    public final /* synthetic */ Object remove(int i11) {
        zzmz();
        zzah(i11);
        double[] dArr = this.zzaxf;
        double d11 = dArr[i11];
        int i12 = this.size;
        if (i11 < i12 - 1) {
            System.arraycopy(dArr, i11 + 1, dArr, i11, (i12 - i11) - 1);
        }
        this.size--;
        this.modCount++;
        return Double.valueOf(d11);
    }
}
