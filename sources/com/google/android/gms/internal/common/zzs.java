...
public final class zzs {
    ...
    public static int zza(int i11, int i12, String str) {
        ...
        if (i11 < 0) {
            str2 = zzy.zza("%s (%s) must not be negative", FirebaseAnalytics.Param.INDEX, Integer.valueOf(i11));
        } else if (i12 < 0) {
            throw new IllegalArgumentException("negative size: " + i12);
        } else {
            str2 = zzy.zza("%s (%s) must be less than size (%s)", FirebaseAnalytics.Param.INDEX, Integer.valueOf(i11), Integer.valueOf(i12));
        }
