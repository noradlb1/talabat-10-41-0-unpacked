...
public final class zzu {
    public static int zza(int i11, int i12, String str) {
        ...
        if (i11 < 0) {
            str2 = zzac.zzb("%s (%s) must not be negative", FirebaseAnalytics.Param.INDEX, Integer.valueOf(i11));
        } else if (i12 < 0) {
            throw new IllegalArgumentException("negative size: " + i12);
        } else {
            str2 = zzac.zzb("%s (%s) must be less than size (%s)", FirebaseAnalytics.Param.INDEX, Integer.valueOf(i11), Integer.valueOf(i12));
        }
    ...
    public static void zzc(int i11, int i12, int i13) {
        ...
        if (i11 < 0 || i12 < i11 || i12 > i13) {
            if (i11 < 0 || i11 > i13) {
                ...
                str = zzac.zzb("end index (%s) must not be less than start index (%s)", Integer.valueOf(i12), Integer.valueOf(i11));
            }
    ...
    private static String zzd(int i11, int i12, String str) {
        if (i11 < 0) {
            return zzac.zzb("%s (%s) must not be negative", str, Integer.valueOf(i11));
        } else if (i12 >= 0) {
            return zzac.zzb("%s (%s) must not be greater than size (%s)", str, Integer.valueOf(i11), Integer.valueOf(i12));
        } else {
            throw new IllegalArgumentException("negative size: " + i12);
        }
    }
