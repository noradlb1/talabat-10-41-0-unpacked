package com.google.android.gms.internal.p002firebaseauthapi;

import java.security.GeneralSecurityException;
import java.util.Arrays;
import java.util.logging.Level;

/* renamed from: com.google.android.gms.internal.firebase-auth-api.zzif  reason: invalid package */
final class zzif implements zzbj {
    private final zzcl zza;
    private final zzoi zzb;
    private final zzoi zzc;

    public zzif(zzcl zzcl) {
        zzoi zzoi;
        this.zza = zzcl;
        if (zzcl.zzf()) {
            zzoj zzb2 = zzku.zza().zzb();
            zzoo zza2 = zzkr.zza(zzcl);
            this.zzb = zzb2.zza(zza2, "daead", "encrypt");
            zzoi = zzb2.zza(zza2, "daead", "decrypt");
        } else {
            zzoi = zzkr.zza;
            this.zzb = zzoi;
        }
        this.zzc = zzoi;
    }

    public final byte[] zza(byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        int length = bArr.length;
        if (length > 5) {
            byte[] copyOf = Arrays.copyOf(bArr, 5);
            byte[] copyOfRange = Arrays.copyOfRange(bArr, 5, length);
            for (zzch zzch : this.zza.zze(copyOf)) {
                try {
                    byte[] zza2 = ((zzbj) zzch.zze()).zza(copyOfRange, bArr2);
                    zzch.zza();
                    int length2 = copyOfRange.length;
                    return zza2;
                } catch (GeneralSecurityException e11) {
                    zzig.zza.logp(Level.INFO, "com.google.crypto.tink.daead.DeterministicAeadWrapper$WrappedDeterministicAead", "decryptDeterministically", "ciphertext prefix matches a key, but cannot decrypt: ".concat(e11.toString()));
                }
            }
        }
        for (zzch zzch2 : this.zza.zze(zzbi.zza)) {
            try {
                byte[] zza3 = ((zzbj) zzch2.zze()).zza(bArr, bArr2);
                zzch2.zza();
                return zza3;
            } catch (GeneralSecurityException unused) {
            }
        }
        throw new GeneralSecurityException("decryption failed");
    }
}
