...
public class KeysDownloader {
    ...
    private boolean hasNonExpiredDataCached() {
        ...
        boolean z12;
        long currentTimeInMillis = getCurrentTimeInMillis();
        long j11 = this.cachedTimeInMillis;
        if (j11 > currentTimeInMillis) {
            z11 = true;
        } else {
            z11 = false;
        }
        if (j11 + this.cacheExpirationDurationInMillis <= currentTimeInMillis) {
            z12 = true;
        } else {
            z12 = false;
        }
    ...
    public long getExpirationDurationInSeconds(HttpHeaders httpHeaders) {
        ...
        if (httpHeaders.getCacheControl() != null) {
            ...
            while (true) {
                ...
                Matcher matcher = MAX_AGE_PATTERN.matcher(split[i11]);
                if (matcher.matches()) {
