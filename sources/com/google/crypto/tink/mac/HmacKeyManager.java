...
public final class HmacKeyManager extends KeyTypeManager<HmacKey> {
    ...
    public HmacKeyManager() {
        super(HmacKey.class, new PrimitiveFactory<Mac, HmacKey>(Mac.class) {
            public Mac getPrimitive(HmacKey hmacKey) throws GeneralSecurityException {
                ...
                int i11 = AnonymousClass3.$SwitchMap$com$google$crypto$tink$proto$HashType[hash.ordinal()];
                if (i11 == 1) {
    ...
    public static KeyTypeManager.KeyFactory.KeyFormat<HmacKeyFormat> createKeyFormat(int i11, int i12, HashType hashType, KeyTemplate.OutputPrefixType outputPrefixType) {
        return new KeyTypeManager.KeyFactory.KeyFormat<>((HmacKeyFormat) HmacKeyFormat.newBuilder().setParams((HmacParams) HmacParams.newBuilder().setHash(hashType).setTagSize(i12).build()).setKeySize(i11).build(), outputPrefixType);
    }

    private static KeyTemplate createTemplate(int i11, int i12, HashType hashType) {
        return KeyTemplate.create(new HmacKeyManager().getKeyType(), ((HmacKeyFormat) HmacKeyFormat.newBuilder().setParams((HmacParams) HmacParams.newBuilder().setHash(hashType).setTagSize(i12).build()).setKeySize(i11).build()).toByteArray(), KeyTemplate.OutputPrefixType.TINK);
    }
    ...
    public static void register(boolean z11) throws GeneralSecurityException {
        Registry.registerKeyManager(new HmacKeyManager(), z11);
    }
