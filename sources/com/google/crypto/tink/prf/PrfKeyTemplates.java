...
public final class PrfKeyTemplates {
    ...
    private static KeyTemplate createHmacTemplate(int i11, HashType hashType) {
        return (KeyTemplate) KeyTemplate.newBuilder().setTypeUrl(new HmacPrfKeyManager().getKeyType()).setValue(((HmacPrfKeyFormat) HmacPrfKeyFormat.newBuilder().setParams((HmacPrfParams) HmacPrfParams.newBuilder().setHash(hashType).build()).setKeySize(i11).build()).toByteString()).setOutputPrefixType(OutputPrefixType.RAW).build();
    }
}
