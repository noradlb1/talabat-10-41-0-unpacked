...
public final class HmacPrfKeyManager extends KeyTypeManager<HmacPrfKey> {
    ...
    private static KeyTemplate createTemplate(int i11, HashType hashType) {
        return KeyTemplate.create(new HmacPrfKeyManager().getKeyType(), ((HmacPrfKeyFormat) HmacPrfKeyFormat.newBuilder().setParams((HmacPrfParams) HmacPrfParams.newBuilder().setHash(hashType).build()).setKeySize(i11).build()).toByteArray(), KeyTemplate.OutputPrefixType.RAW);
    }
    ...
    public static void register(boolean z11) throws GeneralSecurityException {
        Registry.registerKeyManager(new HmacPrfKeyManager(), z11);
    }
