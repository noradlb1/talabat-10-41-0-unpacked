...
public final class TreeRangeMap<K extends Comparable, V> implements RangeMap<K, V> {
    ...
    private static <K extends Comparable, V> Range<K> coalesce(Range<K> range, V v11, @CheckForNull Map.Entry<Cut<K>, RangeMapEntry<K, V>> entry) {
        if (entry == null || !entry.getValue().getKey().isConnected(range) || !entry.getValue().getValue().equals(v11)) {
            return range;
        }
