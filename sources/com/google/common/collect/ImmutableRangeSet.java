...
public final class ImmutableRangeSet<C extends Comparable> extends AbstractRangeSet<C> implements Serializable {
    ...
    public final class AsSet extends ImmutableSortedSet<C> {
        ...
        public int size() {
            ...
            if (num == null) {
                ...
                long j11 = 0;
                while (it.hasNext()) {
                    j11 += (long) ContiguousSet.create((Range) it.next(), this.domain).size();
                    if (j11 >= 2147483647L) {
                        break;
                    }
                }
    ...
    private ImmutableList<Range<C>> intersectRanges(final Range<C> range) {
        ...
        if (range.hasLowerBound()) {
            i11 = SortedLists.binarySearch(this.ranges, Range.upperBoundFn(), range.lowerBound, SortedLists.KeyPresentBehavior.FIRST_AFTER, SortedLists.KeyAbsentBehavior.NEXT_HIGHER);
        } else {
            i11 = 0;
        }
        if (range.hasUpperBound()) {
            i12 = SortedLists.binarySearch(this.ranges, Range.lowerBoundFn(), range.upperBound, SortedLists.KeyPresentBehavior.FIRST_PRESENT, SortedLists.KeyAbsentBehavior.NEXT_HIGHER);
        } else {
            i12 = this.ranges.size();
        }
        ...
        return new ImmutableList<Range<C>>() {
            ...
            public Range<C> get(int i11) {
                Preconditions.checkElementIndex(i11, i13);
                if (i11 == 0 || i11 == i13 - 1) {
                    return ((Range) ImmutableRangeSet.this.ranges.get(i11 + i11)).intersection(range);
                }
                return (Range) ImmutableRangeSet.this.ranges.get(i11 + i11);
            }
        };
    }
