...
public final class BloomFilter<T> implements Predicate<T>, Serializable {
    ...
    public interface Strategy extends Serializable {
        <T> boolean mightContain(@ParametricNullness T t11, Funnel<? super T> funnel, int i11, BloomFilterStrategies.LockFreeBitArray lockFreeBitArray);

        int ordinal();

        <T> boolean put(@ParametricNullness T t11, Funnel<? super T> funnel, int i11, BloomFilterStrategies.LockFreeBitArray lockFreeBitArray);
    }
            ...
            try {
                ...
                try {
                    ...
                    for (int i13 = 0; i13 < readInt; i13++) {
                        lockFreeBitArray.putData(i13, dataInputStream.readLong());
                    }
                    return new BloomFilter<>(lockFreeBitArray, i12, funnel2, bloomFilterStrategies);
    ...
    public boolean mightContain(@ParametricNullness T t11) {
        return this.strategy.mightContain(t11, this.funnel, this.numHashFunctions, this.bits);
    }

    @CanIgnoreReturnValue
    public boolean put(@ParametricNullness T t11) {
    ...
    public void putAll(BloomFilter<T> bloomFilter) {
        ...
        Preconditions.checkArgument(z11, "Cannot combine a BloomFilter with itself.");
        ...
        Preconditions.checkArgument(z12, "BloomFilters must have the same number of hash functions (%s != %s)", i11, i12);
        if (bitSize() == bloomFilter.bitSize()) {
            z13 = true;
        } else {
            z13 = false;
        }
        Preconditions.checkArgument(z13, "BloomFilters must have the same size underlying bit arrays (%s != %s)", bitSize(), bloomFilter.bitSize());
