    ...
    public void onReceive(@Nullable Context context, @Nullable Intent intent) {
        ...
        if (set != null) {
            for (String next : set) {
                ...
                bundle2.putString(new Regex("[ -]*$").replace((CharSequence) new Regex("^[ -]*").replace((CharSequence) new Regex("[^0-9a-zA-Z _-]").replace((CharSequence) next, SideMenuRewardViewHolder.DEFAULT_LOGGED_IN_USER_POINTS_VALUE), ""), ""), (String) bundle.get(next));
            }
        }
