...
public class SddCheckoutMethodFactory extends CheckoutMethodFactory {
    ...
    public Callable<List<CheckoutMethod>> initCheckoutMethods(@NonNull PaymentSession paymentSession) {
        final PaymentMethod findByType = PaymentMethodImpl.findByType(paymentSession.getPaymentMethods(), PaymentMethodTypes.SEPA_DIRECT_DEBIT);
        final Application application = getApplication();
        if (findByType == null) {
            return null;
        }
        PaymentMethodHandler.Factory factory = SepaDirectDebitHandler.FACTORY;
        ...
        return new Callable<List<CheckoutMethod>>() {
            public List<CheckoutMethod> call() {
                return Collections.singletonList(new SddCheckoutMethod(application, findByType));
            }
        };
    }
