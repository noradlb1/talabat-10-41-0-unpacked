...
public class TimerCycle {
    ...
    public class a implements Runnable {
        ...
        public final void run() {
            TimerCycle.this.logger.verbose("%s fired", TimerCycle.this.f40023name);
    ...
    public void start() {
        if (!this.isPaused) {
            this.logger.verbose("%s is already started", this.f40023name);
            return;
        }
        this.logger.verbose("%s starting", this.f40023name);
    ...
    public void suspend() {
        if (this.isPaused) {
            this.logger.verbose("%s is already suspended", this.f40023name);
            return;
        }
        ...
        this.logger.verbose("%s suspended with %s seconds left", this.f40023name, format);
        this.isPaused = true;
    }
