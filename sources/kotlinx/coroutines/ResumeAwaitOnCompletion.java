...
final class ResumeAwaitOnCompletion<T> extends JobNode {
    ...
    public void invoke(@Nullable Throwable th2) {
        ...
        if (state$kotlinx_coroutines_core instanceof CompletedExceptionally) {
            ...
            cancellableContinuationImpl.resumeWith(Result.m6329constructorimpl(ResultKt.createFailure(((CompletedExceptionally) state$kotlinx_coroutines_core).cause)));
            return;
        }
        CancellableContinuationImpl<T> cancellableContinuationImpl2 = this.continuation;
        Result.Companion companion2 = Result.Companion;
        cancellableContinuationImpl2.resumeWith(Result.m6329constructorimpl(JobSupportKt.unboxState(state$kotlinx_coroutines_core)));
    }
}
