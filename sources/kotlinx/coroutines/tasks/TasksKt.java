...
public final class TasksKt {
    ...
    private static final <T> Deferred<T> asDeferredImpl(Task<T> task, CancellationTokenSource cancellationTokenSource) {
        ...
        if (task.isComplete()) {
            ...
            task.addOnCompleteListener((Executor) DirectExecutor.INSTANCE, (OnCompleteListener<T>) new a(CompletableDeferred$default));
        }
    ...
    public static final void m7865asDeferredImpl$lambda0(CompletableDeferred completableDeferred, Task task) {
        Exception exception = task.getException();
        if (exception != null) {
    ...
    public static final <T> Object awaitImpl(Task<T> task, CancellationTokenSource cancellationTokenSource, Continuation<? super T> continuation) {
        if (task.isComplete()) {
            ...
            task.addOnCompleteListener((Executor) DirectExecutor.INSTANCE, (OnCompleteListener<T>) new TasksKt$awaitImpl$2$1(cancellableContinuationImpl));
            if (cancellationTokenSource != null) {
