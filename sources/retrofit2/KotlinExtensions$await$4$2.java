...
public final class KotlinExtensions$await$4$2 implements Callback<T> {
    ...
    public void onResponse(@NotNull Call<T> call, @NotNull Response<T> response) {
        ...
        if (response.isSuccessful()) {
            this.$continuation.resumeWith(Result.m6329constructorimpl(response.body()));
            return;
        }
        CancellableContinuation cancellableContinuation = this.$continuation;
        HttpException httpException = new HttpException(response);
        Result.Companion companion = Result.Companion;
        cancellableContinuation.resumeWith(Result.m6329constructorimpl(ResultKt.createFailure(httpException)));
    }
}
