...
public class NetworkUnmeteredController extends ConstraintController<NetworkState> {
    ...
    public boolean a(@NonNull WorkSpec workSpec) {
        if (workSpec.constraints.getRequiredNetworkType() == NetworkType.UNMETERED || (Build.VERSION.SDK_INT >= 30 && workSpec.constraints.getRequiredNetworkType() == NetworkType.TEMPORARILY_UNMETERED)) {
            return true;
        }
        return false;
    }
