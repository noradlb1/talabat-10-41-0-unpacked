...
public class BrowserActionsIntent {
        ...
        if (list.size() == 1) {
            ...
            if (resolveActivity != null) {
                ...
                while (true) {
                    if (i11 >= list.size()) {
                        break;
                    } else if (str.equals(list.get(i11).activityInfo.packageName)) {
    ...
    public static List<BrowserActionItem> parseBrowserActionItems(@NonNull ArrayList<Bundle> arrayList) {
        ...
        for (int i11 = 0; i11 < arrayList.size(); i11++) {
            ...
            int i12 = bundle.getInt(KEY_ICON_ID);
            Uri uri = (Uri) bundle.getParcelable(KEY_ICON_URI);
            if (TextUtils.isEmpty(string) || pendingIntent == null) {
                throw new IllegalArgumentException("Custom item should contain a non-empty title and non-null intent.");
            }
            if (i12 != 0) {
                browserActionItem = new BrowserActionItem(string, pendingIntent, i12);
            } else {
    ...
    public static void openBrowserAction(@NonNull Context context, @NonNull Uri uri, int i11, @NonNull ArrayList<BrowserActionItem> arrayList, @NonNull PendingIntent pendingIntent) {
        launchIntent(context, new Builder(context, uri).setUrlType(i11).setCustomItems(arrayList).setOnItemSelectedAction(pendingIntent).build().getIntent());
    }
    ...
    public static final class Builder {
        ...
        public Builder setCustomItems(@NonNull ArrayList<BrowserActionItem> arrayList) {
            if (arrayList.size() <= 5) {
                for (int i11 = 0; i11 < arrayList.size(); i11++) {
                    if (TextUtils.isEmpty(arrayList.get(i11).getTitle()) || arrayList.get(i11).getAction() == null) {
                        throw new IllegalArgumentException("Custom item should contain a non-empty title and non-null intent.");
                    }
                    this.mMenuItems.add(getBundleFromItem(arrayList.get(i11)));
                    if (arrayList.get(i11).getIconUri() != null) {
                        this.mImageUris.add(arrayList.get(i11).getIconUri());
                    }
                }
                return this;
            }
