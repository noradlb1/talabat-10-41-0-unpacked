...
final class SplineBasedFloatDecayAnimationSpec implements FloatDecayAnimationSpec {
    ...
    private final double getSplineDeceleration(float f11) {
        return AndroidFlingSpline.INSTANCE.deceleration(f11, WindowInsetsConnection_androidKt.PlatformFlingScrollFriction * this.magicPhysicalCoefficient);
    }

    public final float flingDistance(float f11) {
        return ((float) (((double) (WindowInsetsConnection_androidKt.PlatformFlingScrollFriction * this.magicPhysicalCoefficient)) * Math.exp((WindowInsetsConnection_androidKt.DecelerationRate / WindowInsetsConnection_androidKt.DecelMinusOne) * getSplineDeceleration(f11)))) * Math.signum(f11);
    }
    ...
    public long getDurationNanos(float f11, float f12) {
        return (long) (Math.exp(getSplineDeceleration(f12) / WindowInsetsConnection_androidKt.DecelMinusOne) * 1.0E9d);
    }
    ...
    public float getValueFromNanos(long j11, float f11, float f12) {
        ...
        if (durationNanos > 0) {
            f13 = ((float) j11) / ((float) durationNanos);
        } else {
            f13 = 1.0f;
        }
        return f11 + (flingDistance(f12) * AndroidFlingSpline.FlingResult.m389getDistanceCoefficientimpl(AndroidFlingSpline.INSTANCE.m384flingPositionLfoxSSI(f13)));
    }
    ...
    public float getVelocityFromNanos(long j11, float f11, float f12) {
        ...
        return ((AndroidFlingSpline.FlingResult.m390getVelocityCoefficientimpl(AndroidFlingSpline.INSTANCE.m384flingPositionLfoxSSI(f13)) * flingDistance(f12)) / ((float) durationNanos)) * 1.0E9f;
    }
}
