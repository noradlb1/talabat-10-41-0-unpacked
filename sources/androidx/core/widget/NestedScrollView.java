...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
    ...
    private float getSplineFlingDistance(int i11) {
        double log = Math.log((double) ((((float) Math.abs(i11)) * INFLEXION) / (this.mPhysicalCoeff * SCROLL_FRICTION)));
    ...
    private boolean isWithinDeltaOfScreen(View view, int i11, int i12) {
        ...
        if (this.mTempRect.bottom + i11 < getScrollY() || this.mTempRect.top - i11 > getScrollY() + i12) {
            return false;
        }
        return true;
    }
    ...
    private void onNestedScrollInternal(int i11, int i12, @Nullable int[] iArr) {
        ...
        this.mChildHelper.dispatchNestedScroll(0, scrollY2, 0, i11 - scrollY2, (int[]) null, i12, iArr);
    }
    ...
    private boolean scrollAndFocus(int i11, int i12, int i13) {
        ...
        View findFocusableViewInBounds = findFocusableViewInBounds(z11, i12, i13);
    ...
    private boolean shouldAbsorb(@NonNull EdgeEffect edgeEffect, int i11) {
        ...
        if (getSplineFlingDistance(-i11) < EdgeEffectCompat.getDistance(edgeEffect) * ((float) getHeight())) {
            return true;
        }
        return false;
    }
    ...
    public int computeScrollDeltaToGetChildRectOnScreen(Rect rect) {
        ...
        if (i15 > i11 && rect.top > scrollY) {
            ...
            return Math.min(i13 + 0, (childAt.getBottom() + layoutParams.bottomMargin) - i14);
        } else if (rect.top >= scrollY || i15 >= i11) {
            return 0;
        } else {
            if (rect.height() > height) {
    ...
    public int consumeFlingInVerticalStretch(int i11) {
        ...
        if (i11 > 0 && EdgeEffectCompat.getDistance(this.mEdgeGlowTop) != 0.0f) {
            int round = Math.round((((float) (-height)) / 4.0f) * EdgeEffectCompat.onPullDistance(this.mEdgeGlowTop, (((float) (-i11)) * 4.0f) / ((float) height), 0.5f));
            if (round != i11) {
                this.mEdgeGlowTop.finish();
            }
            return i11 - round;
        } else if (i11 >= 0 || EdgeEffectCompat.getDistance(this.mEdgeGlowBottom) == 0.0f) {
            return i11;
        } else {
            float f11 = (float) height;
            int round2 = Math.round((f11 / 4.0f) * EdgeEffectCompat.onPullDistance(this.mEdgeGlowBottom, (((float) i11) * 4.0f) / f11, 0.5f));
            if (round2 != i11) {
    ...
    public boolean overScrollByCompat(int i11, int i12, int i13, int i14, int i15, int i16, int i17, int i18, boolean z11) {
        ...
        if (z17 && !hasNestedScrollingParent(1)) {
            this.mScroller.springBack(i22, i23, 0, 0, 0, getScrollRange());
        }
