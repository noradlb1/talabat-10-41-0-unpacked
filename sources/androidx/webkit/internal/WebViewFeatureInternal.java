...
public class WebViewFeatureInternal {
    public static final ApiFeature.T ALGORITHMIC_DARKENING = new ApiFeature.T("ALGORITHMIC_DARKENING", "ALGORITHMIC_DARKENING") {
    ...
    public static boolean isStartupFeatureSupported(@NonNull String str, @NonNull Collection<StartupApiFeature> collection, @NonNull Context context) {
        ...
        for (StartupApiFeature next : collection) {
            if (next.getPublicFeatureName().equals(str)) {
                hashSet.add(next);
            }
        }
        if (!hashSet.isEmpty()) {
            for (StartupApiFeature isSupported : hashSet) {
                if (isSupported.isSupported(context)) {
                    return true;
                }
            }
            return false;
