...
public enum SubscriptionHelper implements Subscription {
    ...
    public static void deferredRequest(AtomicReference<Subscription> atomicReference, AtomicLong atomicLong, long j11) {
        Subscription subscription = atomicReference.get();
        if (subscription != null) {
            subscription.request(j11);
        } else if (validate(j11)) {
            BackpressureHelper.add(atomicLong, j11);
    ...
    public static boolean replace(AtomicReference<Subscription> atomicReference, Subscription subscription) {
        ...
        } while (!b.a(atomicReference, subscription2, subscription));
        return true;
    }

    public static void reportMoreProduced(long j11) {
        RxJavaPlugins.onError(new ProtocolViolationException("More produced than requested: " + j11));
    }
    ...
    public static boolean setOnce(AtomicReference<Subscription> atomicReference, Subscription subscription, long j11) {
        if (!setOnce(atomicReference, subscription)) {
            return false;
        }
        subscription.request(j11);
