...
public final class CallServerInterceptor implements Interceptor {
    ...
    public Response intercept(@NotNull Interceptor.Chain chain) throws IOException {
        ...
        if (!HttpMethod.permitsRequestBody(request$okhttp.method()) || body == null) {
            ...
            if (StringsKt__StringsJVMKt.equals("100-continue", request$okhttp.header(HttpHeaders.EXPECT), true)) {
        ...
        if (!this.forWebSocket || code != 101) {
            if (!(build instanceof Response.Builder)) {
                builder2 = build.newBuilder();
            } else {
                builder2 = OkHttp3Instrumentation.newBuilder((Response.Builder) build);
            }
            ResponseBody openResponseBody = exchange$okhttp.openResponseBody(build);
            if (!(builder2 instanceof Response.Builder)) {
                builder3 = builder2.body(openResponseBody);
            } else {
                builder3 = OkHttp3Instrumentation.body(builder2, openResponseBody);
            }
            response = builder3.build();
        } else {
            if (!(build instanceof Response.Builder)) {
                builder4 = build.newBuilder();
            } else {
                builder4 = OkHttp3Instrumentation.newBuilder((Response.Builder) build);
            }
            ResponseBody responseBody = Util.EMPTY_RESPONSE;
            if (!(builder4 instanceof Response.Builder)) {
                builder5 = builder4.body(responseBody);
            } else {
                builder5 = OkHttp3Instrumentation.body(builder4, responseBody);
            }
            response = builder5.build();
        }
        ...
        if (code == 204 || code == 205) {
            ...
            if (j11 > 0) {
                ...
                if (body3 != null) {
                    l11 = Long.valueOf(body3.contentLength());
                }
                sb2.append(l11);
