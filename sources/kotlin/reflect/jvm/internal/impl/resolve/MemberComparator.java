...
public class MemberComparator implements Comparator<DeclarationDescriptor> {
    ...
    public int compare(DeclarationDescriptor declarationDescriptor, DeclarationDescriptor declarationDescriptor2) {
        Integer a11 = NameAndTypeMemberComparator.compareInternal(declarationDescriptor, declarationDescriptor2);
        if (a11 != null) {
            return a11.intValue();
        }
        if ((declarationDescriptor instanceof TypeAliasDescriptor) && (declarationDescriptor2 instanceof TypeAliasDescriptor)) {
            ...
            for (int i11 = 0; i11 < Math.min(valueParameters.size(), valueParameters2.size()); i11++) {
                ...
                int compareTo3 = descriptorRenderer3.renderType(valueParameters.get(i11).getType()).compareTo(descriptorRenderer3.renderType(valueParameters2.get(i11).getType()));
                if (compareTo3 != 0) {
                    return compareTo3;
                }
            }
            ...
            for (int i12 = 0; i12 < Math.min(typeParameters.size(), typeParameters2.size()); i12++) {
                ...
                for (int i13 = 0; i13 < upperBounds.size(); i13++) {
                    ...
                    int compareTo4 = descriptorRenderer4.renderType(upperBounds.get(i13)).compareTo(descriptorRenderer4.renderType(upperBounds2.get(i13)));
                    if (compareTo4 != 0) {
                        return compareTo4;
                    }
                }
