    ...
    public void accept(SignatureVisitor signatureVisitor) {
        ...
        if (str.charAt(0) == '<') {
            ...
            do {
                ...
                signatureVisitor.visitFormalTypeParameter(str.substring(i11 - 1, indexOf));
                int i12 = indexOf + 1;
                char charAt2 = str.charAt(i12);
                if (charAt2 == 'L' || charAt2 == '[' || charAt2 == 'T') {
                    i12 = parseType(str, i12, signatureVisitor.visitClassBound());
                }
                while (true) {
                    i11 = i12 + 1;
                    ...
                    i12 = parseType(str, i11, signatureVisitor.visitInterfaceBound());
                }
            } while (charAt != '>');
        }
        if (str.charAt(i11) == '(') {
            ...
            while (str.charAt(i13) != ')') {
                i13 = parseType(str, i13, signatureVisitor.visitParameterType());
            }
            int parseType = parseType(str, i13 + 1, signatureVisitor.visitReturnType());
            while (parseType < length) {
                parseType = parseType(str, parseType + 1, signatureVisitor.visitExceptionType());
            }
            return;
        }
        int parseType2 = parseType(str, i11, signatureVisitor.visitSuperclass());
