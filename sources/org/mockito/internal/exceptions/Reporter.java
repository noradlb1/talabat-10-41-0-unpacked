...
public class Reporter {
    ...
    public static AssertionError argumentsAreDifferent(String str, List<String> list, List<Location> list2) {
        ...
        sb3.append("Argument(s) are different! Wanted:\n");
        sb3.append(str);
        sb3.append(StringUtils.LF);
        sb3.append(new LocationImpl());
        sb3.append(StringUtils.LF);
        sb3.append("Actual invocations have different arguments:\n");
        ...
        return ExceptionFactory.createArgumentsAreDifferentException(sb3.toString(), str, sb2.toString());
    }
    ...
    public static MockitoException cannotCreateTimerWithNegativeDurationTime(long j11) {
        return new FriendlyReminderException(StringUtil.join("", "Don't panic! I'm just a friendly reminder!", "It is impossible for time to go backward, therefore...", "You cannot put negative value of duration: (" + j11 + ")", "as argument of timer methods (after(), timeout())", ""));
    }
    ...
    public static MockitoException cannotMockClass(Class<?> cls, String str) {
        return new MockitoException(StringUtil.join("Cannot mock/spy " + cls, "Mockito cannot mock/spy because :", " - " + str));
    }
    ...
    public static MockitoException checkedExceptionInvalid(Throwable th2) {
        return new MockitoException(StringUtil.join("Checked exception is invalid for this method!", "Invalid: " + th2));
    }
    ...
    private static String createAllLocationsArgsMessage(List<Invocation> list) {
        ...
        for (Invocation next : list) {
            ...
            if (location != null) {
                ...
                sb2.append(Arrays.toString(next.getArguments()));
                sb2.append(StringUtils.LF);
            }
        }
        return sb2.toString();
    ...
    private static String createTooFewInvocationsMessage(Discrepancy discrepancy, DescribedInvocation describedInvocation, List<Location> list) {
        ...
        StringBuilder sb2 = new StringBuilder();
        sb2.append("Wanted ");
        sb2.append(discrepancy.getPluralizedWantedCount());
        int wantedCount = discrepancy.getWantedCount();
        String str = RealDiscoveryConfigurationRepository.VERSION_DELIMETER;
        sb2.append(wantedCount == 0 ? str : CertificateUtil.DELIMITER);
        objArr[1] = sb2.toString();
        objArr[2] = new LocationImpl();
        StringBuilder sb3 = new StringBuilder();
        sb3.append("But was ");
        sb3.append(discrepancy.getPluralizedActualCount());
    ...
    private static String createTooManyInvocationsMessage(int i11, int i12, DescribedInvocation describedInvocation, List<Location> list) {
        return StringUtil.join(describedInvocation.toString(), "Wanted " + Pluralizer.pluralize(i11) + CertificateUtil.DELIMITER, new LocationImpl(), "But was " + Pluralizer.pluralize(i12) + CertificateUtil.DELIMITER, createAllLocationsMessage(list), "");
    }
    ...
    public static UnnecessaryStubbingException formatUnncessaryStubbingException(Class<?> cls, Collection<Invocation> collection) {
        ...
        for (Invocation location : collection) {
            ...
            sb2.append(i11);
            sb2.append(". ");
            sb2.append(location.getLocation());
            i11++;
        }
        if (cls != null) {
        ...
        return new UnnecessaryStubbingException(StringUtil.join(str, "Clean & maintainable test code requires zero unnecessary code.", "Following stubbings are unnecessary (click to navigate to relevant line of code):" + sb2, "Please remove unnecessary stubbings or use 'lenient' strictness. More info: javadoc for UnnecessaryStubbingException class."));
    }
    ...
    public static MockitoException inlineClassWithoutUnboxImpl(Class<?> cls, Exception exc) {
        return new MockitoException(StringUtil.join("Kotlin inline class should have unbox-impl() method,", "but " + cls + " does not."), exc);
    }

    public static MockitoException invalidArgumentPositionRangeAtInvocationTime(InvocationOnMock invocationOnMock, boolean z11, int i11) {
        String str;
        Object[] objArr = new Object[7];
        objArr[0] = "Invalid argument index for the current invocation of method : ";
        objArr[1] = " -> " + MockUtil.getMockName(invocationOnMock.getMock()) + RealDiscoveryConfigurationRepository.VERSION_DELIMETER + invocationOnMock.getMethod().getName() + "()";
        objArr[2] = "";
        StringBuilder sb2 = new StringBuilder();
        if (z11) {
            str = "Last parameter wanted";
        } else {
            str = "Wanted parameter at position " + i11;
        }
        sb2.append(str);
        sb2.append(" but ");
        sb2.append(possibleArgumentTypesOf(invocationOnMock));
    ...
    public static MockitoException invalidUseOfMatchers(int i11, List<LocalizedMatcher> list) {
        return new InvalidUseOfMatchersException(StringUtil.join("Invalid use of argument matchers!", i11 + " matchers expected, " + list.size() + " recorded:" + locationsOf(list), "", "This exception may occur if matchers are combined with raw values:", "    //incorrect:", "    someMethod(any(), \"raw String\");", "When using matchers, all arguments have to be provided by matchers.", "For example:", "    //correct:", "    someMethod(any(), eq(\"String by matcher\"));", "", "For more info see javadoc for Matchers class.", ""));
    }

    public static MockitoException invocationListenerThrewException(InvocationListener invocationListener, Throwable th2) {
        return new MockitoException(StringUtil.join("The invocation listener with type " + invocationListener.getClass().getName(), "threw an exception : " + th2.getClass().getName() + th2.getMessage()), th2);
    }
    ...
    public static MockitoAssertionError neverWantedButInvoked(DescribedInvocation describedInvocation, List<Invocation> list) {
        return new NeverWantedButInvoked(StringUtil.join(describedInvocation.toString(), "Never wanted here:", new LocationImpl(), "But invoked here:", createAllLocationsArgsMessage(list)));
    }
    ...
    public static MockitoAssertionError noInteractionsWanted(Object obj, List<VerificationAwareInvocation> list) {
        ...
        ArrayList arrayList = new ArrayList();
        for (VerificationAwareInvocation location : list) {
            arrayList.add(location.getLocation());
        }
        return new NoInteractionsWanted(StringUtil.join("No interactions wanted here:", new LocationImpl(), "But found these interactions on mock '" + MockUtil.getMockName(obj) + "':", StringUtil.join("", arrayList), print));
    }
    ...
    private static StringBuilder possibleArgumentTypesOf(InvocationOnMock invocationOnMock) {
        ...
        for (int i11 = 0; i11 < length; i11++) {
            sb2.append("    [");
            sb2.append(i11);
            if (!invocationOnMock.getMethod().isVarArgs() || i11 != length - 1) {
                sb2.append("] ");
                sb2.append(parameterTypes[i11].getSimpleName());
                sb2.append(StringUtils.LF);
            } else {
                sb2.append("+] ");
                sb2.append(parameterTypes[i11].getComponentType().getSimpleName());
                sb2.append("  <- Vararg");
                sb2.append(StringUtils.LF);
            }
        }
    ...
    public static MockitoAssertionError tooFewActualInvocationsInOrder(Discrepancy discrepancy, DescribedInvocation describedInvocation, List<Location> list) {
        String createTooFewInvocationsMessage = createTooFewInvocationsMessage(discrepancy, describedInvocation, list);
        return new VerificationInOrderFailure(StringUtil.join("Verification in order failure:" + createTooFewInvocationsMessage));
    }

    public static MockitoAssertionError tooManyActualInvocations(int i11, int i12, DescribedInvocation describedInvocation, List<Location> list) {
        return new TooManyActualInvocations(createTooManyInvocationsMessage(i11, i12, describedInvocation, list));
    }

    public static MockitoAssertionError tooManyActualInvocationsInOrder(int i11, int i12, DescribedInvocation describedInvocation, List<Location> list) {
        String createTooManyInvocationsMessage = createTooManyInvocationsMessage(i11, i12, describedInvocation, list);
    ...
    public static MoreThanAllowedActualInvocations wantedAtMostX(int i11, int i12) {
        return new MoreThanAllowedActualInvocations(StringUtil.join("Wanted at most " + Pluralizer.pluralize(i11) + " but was " + i12));
    }
    ...
    public static MockitoException wrongTypeOfArgumentToReturn(InvocationOnMock invocationOnMock, String str, Class<?> cls, int i11) {
        return new WrongTypeOfReturnValue(StringUtil.join("The argument of type '" + cls.getSimpleName() + "' cannot be returned because the following ", "method should return the type '" + str + "'", " -> " + MockUtil.getMockName(invocationOnMock.getMock()) + RealDiscoveryConfigurationRepository.VERSION_DELIMETER + invocationOnMock.getMethod().getName() + "()", "", "The reason for this error can be :", "1. The wanted argument position is incorrect.", "2. The answer is used on the wrong interaction.", "", "Position of the wanted argument is " + i11 + " and " + possibleArgumentTypesOf(invocationOnMock), "***", "However if you're still unsure why you're getting above error read on.", "Due to the nature of the syntax above problem might occur because:", "1. This exception *might* occur in wrongly written multi-threaded tests.", "   Please refer to Mockito FAQ on limitations of concurrency testing.", "2. A spy is stubbed using when(spy.foo()).then() syntax. It is safer to stub spies - ", "   - with doReturn|Throw() family of methods. More in javadocs for Mockito.spy() method.", ""));
    }
