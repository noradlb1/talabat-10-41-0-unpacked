...
public class BcBands extends BandSet {
    ...
    public BcBands(CpBands cpBands2, Segment segment2, int i11) {
    ...
    private List getIndexInClass(List list) {
        ...
        for (int i11 = 0; i11 < list.size(); i11++) {
            arrayList.add(Integer.valueOf(((CPMethodOrField) list.get(i11)).getIndexInClass()));
        }
        return arrayList;
    }
    ...
    private List getIndexInClassForConstructor(List list) {
        ArrayList arrayList = new ArrayList(list.size());
        for (int i11 = 0; i11 < list.size(); i11++) {
            arrayList.add(Integer.valueOf(((CPMethodOrField) list.get(i11)).getIndexInClassForConstructor()));
        }
        return arrayList;
    }
    ...
    public void visitEnd() {
        for (int i11 = 0; i11 < this.bciRenumbering.size(); i11++) {
            if (this.bciRenumbering.get(i11) == -1) {
                this.bciRenumbering.remove(i11);
                IntList intList = this.bciRenumbering;
                int i12 = this.renumberedOffset + 1;
                this.renumberedOffset = i12;
                intList.add(i11, i12);
            }
        }
        int i13 = this.renumberedOffset;
        if (i13 == 0) {
            return;
        }
        if (i13 + 1 == this.bciRenumbering.size()) {
    ...
    public void visitIincInsn(int i11, int i12) {
        if (i11 > 255 || i12 > 255) {
            ...
            this.bcLocal.add(i11);
            this.bcShort.add(i12);
        } else {
            this.byteCodeOffset += 3;
            this.bcCodes.add(132);
            this.bcLocal.add(i11);
            this.bcByte.add(i12 & 255);
        }
        updateRenumbering();
    }
    ...
    public void visitLookupSwitchInsn(Label label, int[] iArr, Label[] labelArr) {
        ...
        for (int i12 = 0; i12 < labelArr.length; i12++) {
            this.bcCaseValue.add(iArr[i12]);
            this.bcLabel.add(labelArr[i12]);
            this.bcLabelRelativeOffsets.add(this.byteCodeOffset);
        }
        ...
        this.byteCodeOffset = i13 + i11 + 1 + 8 + (iArr.length * 8);
        updateRenumbering();
    }
    ...
    public void visitTableSwitchInsn(int i11, int i12, Label label, Label[] labelArr) {
        ...
        int i14 = this.byteCodeOffset;
        if (i14 % 4 != 0) {
            i13 = 4 - (i14 % 4);
        }
        this.byteCodeOffset = i14 + i13 + 12 + (labelArr.length * 4);
        updateRenumbering();
    }
