...
class BlockOutputStream extends FinishableOutputStream {
    ...
    public BlockOutputStream(OutputStream outputStream, FilterEncoder[] filterEncoderArr, Check check2, ArrayCache arrayCache) throws IOException {
        ...
        for (int i11 = 0; i11 < filterEncoderArr.length; i11++) {
            EncoderUtil.encodeVLI(byteArrayOutputStream, filterEncoderArr[i11].getFilterID());
            byte[] filterProps = filterEncoderArr[i11].getFilterProps();
    ...
    public void write(byte[] bArr, int i11, int i12) throws IOException {
        this.filterChain.write(bArr, i11, i12);
        this.check.update(bArr, i11, i12);
        this.uncompressedSize += (long) i12;
        validate();
    }
}
