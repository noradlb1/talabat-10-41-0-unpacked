...
public final class AndroidInjection {
    ...
    public static void inject(Fragment fragment) {
        ...
        if (Log.isLoggable(TAG, 3)) {
            String.format("An injector for %s was found in %s", new Object[]{fragment.getClass().getCanonicalName(), findHasAndroidInjectorForFragment.getClass().getCanonicalName()});
        }
        inject((Object) fragment, findHasAndroidInjectorForFragment);
    }

    public static void inject(Service service2) {
        Preconditions.checkNotNull(service2, "service");
        Application application = service2.getApplication();
        if (application instanceof HasAndroidInjector) {
            inject((Object) service2, (HasAndroidInjector) application);
        } else {
            throw new RuntimeException(String.format("%s does not implement %s", new Object[]{application.getClass().getCanonicalName(), HasAndroidInjector.class.getCanonicalName()}));
        }
    }
